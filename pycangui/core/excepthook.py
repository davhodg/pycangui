...
class ExceptionLogger(QObject):
    ...
    def _thread_hook(self, args) -> None:
        ...
        self.caught.emit(f"on thread {name}:\n{text}")
        if self._previous_thread is not None:
    ...
    def _deliver(self, text: str) -> None:
        ...
        self._seen.add(key)
        self._sink(
            "Unhandled error (this is a bug in pycangui; the action it "
            f"interrupted did not finish):\n{text}"
        )
